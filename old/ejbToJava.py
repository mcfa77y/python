...
def parse_finders(text):
    ...
    for findermatch in get_all_finders(text):
    	out.writeTo(finder_to_java(findermatch))
 ...
 *      query="select object(c) from TaskGroup as c where c.deleteStatus = 0"
 * @ejb.finder
 *      signature="TaskGroup findByAlphaId(java.lang.String alphaId, java.lang.Integer rootObjectId, java.lang.Integer rootObjectType)"
 *      query="select object(c) from TaskGroup as c
 *             where c.alphaId = ?1
 *             and c.rootObjectId = ?2
 *             and c.rootObjectType = ?3
 *             and c.deleteStatus = 0"
 * @ejb.finder
 *      signature="TaskGroup findByName(java.lang.String name, java.lang.Integer rootObjectId, java.lang.Integer rootObjectType)"
 *      query="select object(c) from TaskGroup as c
 *             where c.name = ?1
 *             and c.rootObjectId = ?2
 *             and c.rootObjectType = ?3
 *             and c.deleteStatus = 0"
 * @ejb.finder
 *      signature="java.util.Collection findByBusinessObject(java.lang.Integer rootObjectId, java.lang.Integer rootObjectType)"
 *      query="select object(c) from TaskGroup as c where c.rootObjectId = ?1 and c.rootObjectType = ?2 and c.deleteStatus = 0"
 *
 '''
	parse_finders(text)
main()
