...
def main():
	...
	# step 10
	sr.printStep(10)
	sr.replace(r'(public\s+[^{}();]*\s+(get[A-Za-z0-9_]*|[A-Za-z0-9_]*HasBeenSet)\s*\(\s*\))',r'@Transient\n\1', DO_MULTIPLE_REPLACE)
	sr.doImport("import javax.persistence.Transient;");
	# step 11
	sr.printStep(11)
	go = sr.replace(r'(public (\w+)DTO get.+)',r'@ManyToOne\n//TODO: enter column name\n@JoinColumn(name="\2Id")\n\1',DO_MULTIPLE_REPLACE)
	sr.replace('@Transient\s@ManyToOne','@ManyToOne',DO_MULTIPLE_REPLACE)
	if go:
	...
	# step 13
	sr.printStep(13)
	sr.replace('addValueToMap', 'resetLocalization',DO_MULTIPLE_REPLACE)
	
	# change StateFulBusinesSObject to Stateful
	sr.printStep("StateFulBusinesSObject to Stateful")
	sr.replace('com.bricsnet.core.util.model.StatefulBusinessObject',"com.bricsnet.core.util.model.type.Stateful",not DO_MULTIPLE_REPLACE)
