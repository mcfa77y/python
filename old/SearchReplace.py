...
class SearchReplace():
	...
	def findInsert (self,searchTerm, insertTerm, offset, isMulti):
		...
		for i,line in enumerate(self.listFile):
				s = re.search(searchTerm,line)
				if s:
					print "FindInsert - Search Term Found: "
					print s.group()+"\tline:"+line
					self.listFile.insert(i+offset,insertTerm)
					print insertTerm+" Inserted"
					if(not isMulti):
						return True
					else:
						found = True
	...
	def findGet(self, searchTerm, isMulti):
		...
		for i,line in enumerate(self.listFile):
			...
			if pResult:
				if not isMulti:
					return (pResult.group(1),i)
				else:
					getList.append((pResult.group(1),i))
		return getList
	def getBetween(self, index, startTerm, endTerm):
		...
		if startIndex<endIndex and (endIndex-startIndex)<10:
			for i,line in enumerate(self.listFile[startIndex:endIndex]):
				getList.append(line)
		return getList
