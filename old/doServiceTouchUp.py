...
def main():
	...
	sr.replace(r"(\w+)Util\.getLocalHome\(\)\.findByPrimaryKey\((\w+)\)\.getAlphaId\(\)",r"new (\1)Service().getDTO(\2).getAlphaId()", DO_MULTIPLE_REPLACE)
	
	sr.replace(r"(\w+)Local ejb = \w+Util\.getLocalHome\(\)\.findByPrimaryKey\((\w+)\);\s*(\w+ \w+ = )ejb\.get\w+DTO\(\);",r"\3=new \1Service().getDTO(\2)",DO_MULTIPLE_REPLACE)
