...
def main():
	...
	# step 5
	sr.printStep(5)
	go = sr.replace(r'public class (\w+)DTO',r'@Entity\n@Table(name="\1")\npublic class \1DTO', not DO_MULTIPLE_REPLACE)
	if go:
	...
	# step 6
	sr.printStep(6)
	sr.replace(r'(public .+ getId\(\))',r'@Id\n\1', not DO_MULTIPLE_REPLACE)
	sr.doImport("import javax.persistence.Id;");
	# step 7
	sr.printStep(7)
	#go = sr.replace(r'(Auditable)',r'\1', not DO_MULTIPLE_REPLACE)
	go = sr.checkImplements('Auditable');
	print "Is auditable: "+str(go)
	
	if go:
		sr.replace(r'(merge\(\))',r'//TODO:DTO is Auditable, delete the lines in the merge method that are showing errors and leave the rest.\n \1', not DO_MULTIPLE_REPLACE)
		print "DTO is Auditable, delete the lines in the merge method that are showing errors and leave the rest. "
	else:
		sr.replace(r'(public int hashCode\(\))',r'//TODO:DTO is not Auditable, remove everything below the hashCode declaration.\n \1', not DO_MULTIPLE_REPLACE)
		sr.replace(r'(protected .+ historyProperties .+)',r'//TODO:Remove History Properties.\n\1',not DO_MULTIPLE_REPLACE)
