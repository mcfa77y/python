...
def example():
    """Example of methods' intended use.

    The path names would have to be changed to be useful on a particular
    machine, of course.
    """
    trunk = parse_report_dir(r'C:\logs\TrunkCactus20110625\report')
    for d in ['HibernateCactus20110705',]:
        cases = parse_report_dir('C:/logs/{0}/report'.format(d))
        keys_to_fix = measure_regression(trunk, cases)
        to_fix = [cases[k] for k in keys_to_fix]
        with open('C:/logs/{0}/ReportByErrorMessage.txt'.format(d), mode='w') as f:
            print_cases_as_todo_list(to_fix, f)

def regressionTest():
	list_regressions(r'C:\logs\HibernateCactus20110629\report', r'C:\logs\HibernateCactus20110705\report')
