...
def main():
	...
	# step 2 
	sr.printStep(2)
	sr.regExReplace(r'(\w+)Home home = (\w+)Util.getLocalHome\(\);',r'', DO_MULTIPLE_REPLACE)
	# step 3
	sr.printStep(3)
	sr.regExReplace(r'home\.findByAlphaId\(alphaId\).get(\w+)DTO\(\)',r'findByAlphaId(alphaId)', DO_MULTIPLE_REPLACE)
	# step 4
	sr.printStep(4)
	sr.regExReplace(r'home\.findByPrimaryKey\((\w+)\)\.get(\w+)DTO\(\)',r'new \2Service().getDTO(\1)', DO_MULTIPLE_REPLACE)
	# step 5
	sr.printStep(5)
	sr.regExReplace(r'home\.(.+)',r'\1', DO_MULTIPLE_REPLACE)
